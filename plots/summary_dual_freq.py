...
fig_prefix = 'histograms_dualfreq_w_ssrg_after_burnin_'

path_root_list_dual_freq = [
                  '/home/billault/Documents/In_progress/mcmc/tests_cv_longer_newdata/chains_spectra_pt_dualfreq/mcmc_chains_spectra_XW_edr1e-4_cov_discard_noise_sigma0.35_freeturb_pt_1lev_swap1/',
                  '/home/billault/Documents/In_progress/mcmc/tests_cv_longer_newdata/chains_spectra_pt_dualfreq/mcmc_chains_spectra_KaW_edr1e-4_cov_discard_noise_sigma0.35_freeturb_pt_1lev_swap1/',
                  '/home/billault/Documents/In_progress/mcmc/tests_cv_longer_newdata/chains_spectra_pt_dualfreq/mcmc_chains_spectra_XKa_edr1e-4_cov_discard_noise_sigma0.35_freeturb_pt_1lev_swap1/',
                  '/home/billault/Documents/In_progress/mcmc/tests_cv_longer_newdata/chains_spectra_pt_dualfreq_ssrg/mcmc_chains_spectra_KaW_edr1e-4_cov_discard_noise_sigma0.35_freeturb_pt_1lev_swap1_ori1/',
                  ]
...
label_list = ['XW', 'KaW', 'XKa', 'XW_SSRG_Ori']
...
for target_spec in target_spec_list:
    ...
    hist_list_2f= []  
    for i, path_root in enumerate(path_root_list_dual_freq):
        hist_list_2f.append(compute_hist(path_root, target_spec, verbose=0,after_burnin=after_burnin)[0])

    ground_truth = compute_hist(path_root_list_true[0], target_spec, verbose=0,after_burnin=after_burnin)[1]


    for i in range(len(hist_list_2f)):
        hist_list_2f[i]['Deff'] = 10**hist_list_2f[i]['logDeff']
        hist_list_2f[i]['freqs'] = 'dual'

    # for i in range(len(hist_list_true)):
    hist_list_true['Deff'] = 10**hist_list_true['logDeff']
    hist_list_true['freqs'] = 'XKaW'

    hist_list_joint = pd.concat(hist_list_2f+[hist_list_true]).reset_index(drop=True)
    # hist_list_joint = pd.concat(hist_list_2f+hist_list_true).reset_index(drop=True)
    ...
    for var in vars_to_plot.keys():
        ...
        if var=='iwc':
            ...
            bins=np.linspace(0,np.nanmax(hist_list_joint['Deff']))
        else:
            bins = np.linspace(np.nanmin(hist_list_joint[var]),np.nanmax(hist_list_joint[var]),100)
        ymax = -1
        for i2f in range(len(hist_list_2f)):
            n2f,b2f = np.histogram(hist_list_2f[i2f][var], bins=bins,density=d)
            axs[i_v].step(n2f,b2f[1:],linewidth=2.5,color=color_list[i2f])
            ymax = max(n2f.max(),ymax)
